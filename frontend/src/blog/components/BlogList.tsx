	...
	return (
		<>
			<article className="flex flex-wrap container mt-20 gap-6">
				{blogs?.results?.map((blog: Blog) => (
					<Link to={`/blog/${blog.slug}`} key={blog.slug} className="w-full p-4">
						<div className="bg-zinc-600 rounded-lg shadow p-4 h-full max-w-5xl mx-auto">
							...
							<p>
								{blog.categories.map((t) => (
									<strong key={t.id} className="mx-2">
										{t.title}
									</strong>
								))}
							</p>
							<p className="truncate">{blog.overview}</p>
						</div>
					</Link>
				))}
			</article>
